...
import 'package:admin/screen/order/receipt_screen.dart';
...
class OrderDetailDeliverScreen extends StatefulWidget {
  final Order order;
  const OrderDetailDeliverScreen({super.key, required this.order});

  @override
  State<OrderDetailDeliverScreen> createState() =>
      _OrderDetailDeliverScreenState();
}

class _OrderDetailDeliverScreenState extends State<OrderDetailDeliverScreen> {
  Order? _order;
  @override
  void initState() {
    super.initState();
    ...
    return PopScope(
      ...
      onPopInvokedWithResult: (didPop, result) {
        if (didPop) {
          context
              .read<OrderBloc>()
              .add(OrderGetList(status: OrderStatus.DELIVERED));
        }
      },
                    ...
                    child: ListView(
                      ...
                      children: [
                        ...
                        _buildParcelInfoSection(
                            order.totalAmount ?? 0.0, OrderStatus.DELIVERED),
                        const SizedBox(height: 8),
                        ...products!.map(
                          (e) {
