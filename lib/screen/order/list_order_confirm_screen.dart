...
import 'package:admin/screen/order/order_detail_confirm_screen.dart';
import 'package:cached_network_image/cached_network_image.dart';
import 'package:flutter/material.dart';
import 'package:flutter_bloc/flutter_bloc.dart';
import 'package:intl/intl.dart';

class ListOrderConfirmScreen extends StatefulWidget {
  const ListOrderConfirmScreen({super.key});

  @override
  State<ListOrderConfirmScreen> createState() => _ListOrderConfirmScreenState();
}

class _ListOrderConfirmScreenState extends State<ListOrderConfirmScreen> {
  final currencyFormatter = NumberFormat.currency(locale: 'vi_VN', symbol: 'đ');
  @override
  void initState() {
    // TODO: implement initState
    super.initState();
    context.read<OrderBloc>().add(OrderGetList(status: OrderStatus.CONFIRMED));
  }
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      appBar: AppBar(
        title: const Text('Danh sách đơn hàng đã duyệt'),
        centerTitle: true,
      ...
      body: BlocBuilder<OrderBloc, OrderState>(
        builder: (context, state) {
          ...
          if (state is OrderError) {
            print("Tải đơn hàng confirm thất bại${state.message}");
            return Center(
              child: Text(
                ...
                return InkWell(
                    onTap: () {
                      Navigator.push(context,
                        ...
                        return OrderDetailConfirmScreen(order: orders[index]);
                      }));
                    },
                  ...
                  children: [
                    OutlinedButton(
                      onPressed: () {
                        Navigator.push(context,
                            MaterialPageRoute(builder: (context) {
                          return OrderDetailConfirmScreen(order: order);
                        }));
                      },
                      style: OutlinedButton.styleFrom(
